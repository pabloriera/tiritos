...
The game header changes with the current stage: map browsing and creation actions during selection, invite/readiness controls in the lobby, player status and events during play, result/rematch actions after a match, and a compact Paint-style ribbon in the map studio. Every new design starts as a blank floor. The ribbon provides wall and eraser brushes, explicit spawn and metro stamps, brush presets, player colors, undo, and a recoverable clear action. Use **Preview** to test the same movement, metro, projectile, and destructible-wall behavior used by the game before saving. A playable map needs at least two user-placed spawns; metro stations must be omitted or placed in groups of two or more. Saving returns directly to the new map in the selection sandbox, where it can be tested and used to create a multiplayer room.
